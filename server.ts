...
function getGenreStyleGuide(genre: string): string {
  const g = (genre || "").trim();
...
// --- CIRCUIT BREAKER PATTERN FOR API KEYS (Ngắt mạch API lỗi) ---
const blacklistedKeys = new Map<string, number>();
const BLACKLIST_COOLDOWN_MS = 5 * 60 * 1000; // Thời gian ngắt mạch: 5 phút

// Hàm bổ trợ chủ động tạo độ trễ bảo vệ chuỗi hạn mức API (Exponential Backoff Helper)
const sleep = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));

// Multi-key rotational executor helper với cơ chế Circuit Breaker nâng cao
...
async function checkLeftoverGlossary(
    sourceText: string,
    glossary: any[],
    apiKeys: string[] | undefined,
    modelName: string | undefined,
...
app.post("/api/analyze-glossary", async (req, res) => {
  try {
    ...
    const systemInstruction =
        ...
        "ĐẶC BIỆT LƯU Ý: Nếu từ ngữ gồm tên phiên âm ngoại quốc đi kèm hậu tố danh từ chỉ loại tiếng Trung (như 茶 - trà, 镇 - thị trấn, 河 - sông, 城 - thành), bạn phải dịch danh từ chỉ loại đó sang tiếng Việt và xếp lên trước tên gốc tiếng Anh (Ví dụ: 阿帕茶 dịch thành 'Trà Abbacchio' chứ KHÔNG ĐƯỢC để 'Abbacchio Tea'). " +
...
// Parser đồng bộ Markdown cấu trúc bảng thuật ngữ
function parseGlossaryFromMd(text: string): Array<{
  chinese: string;
  pinyin: string;
  vietnamese: string;
  type: string;
...
app.post("/api/analyze-guidelines", async (req, res) => {
  try {
    ...
    const schema = {
      ...
      properties: {
        genre: {
          ...
          enum: ["Tiên Hiệp", "Võ Hiệp", "Ngôn Tình", "Đô Thị", "Huyền Huyễn", "Huyền Huyễn Phương Tây", "Vô Hạn Lưu", "Khác"],
        },
        tone: {
          type: Type.STRING,
          description: "Tông giọng biên dịch phù hợp nhất",
          enum: ["Dịch thuần Việt mượt mà", "Trang nghiêm cổ phong", "Bình dị dân dã", "Hùng tráng dồn dập"],
        },
        description: {
          type: Type.STRING,
    ...
    const aiMeta = JSON.parse(rotationResult.text);
...
${substitutedText}`;
  ...
  try {
    ...
    // 💡 GIẢI PHÁP NÂNG CAO: Tự động đếm dấu ngoặc để cô lập khối JSON chuẩn xác, loại bỏ text thừa ở đuôi
    try {
      ...
      if (startIdx !== -1) {
        ...
        if (endIdx !== -1) {
          const cleanJsonStr = resultText.substring(startIdx, endIdx + 1);
          parsed = JSON.parse(cleanJsonStr);
        } else {
          // Phương án dự phòng cuối cùng bằng regex nếu chuỗi mất dấu ngoặc kết thúc
      ...
      throw new Error(`Gemma trả về văn bản không thể giải mã JSON: ${resultText.substring(0, 100)}`);
    }
  }
...
async function translateRawWithContentSplit(
  ...
  if (text.length < 150 || depth > 4) {
    return callRawTranslationDirect(text, genre, tone, glossary, apiKeys, model, startKeyIndex);
  }
  ...
  try {
    const result = await callRawTranslationDirect(text, genre, tone, glossary, apiKeys, model, startKeyIndex);
    ...
    if (isSafetyOrEmpty) {
      ...
      const [res1, res2] = await Promise.all([
        translateRawWithContentSplit(part1, genre, tone, glossary, apiKeys, model, depth + 1, startKeyIndex),
        translateRawWithContentSplit(part2, genre, tone, glossary, apiKeys, model, depth + 1, startKeyIndex)
      ]);
      return {
...
async function callPolishDirect(
  ...
  const systemInstruction =
      "Bạn là biên tập viên văn học lâu năm chuyên chuốt văn phong truyện dịch chữ Trung - Việt.\n" +
...
async function polishWithContentSplit(
  ...
  if (rawTranslation.length < 150 || depth > 4) {
    return callPolishDirect(sourceText, rawTranslation, genre, tone, glossary, additionalInstructions, apiKeys, model, startKeyIndex);
  }
  ...
  try {
    const result = await callPolishDirect(sourceText, rawTranslation, genre, tone, glossary, additionalInstructions, apiKeys, model, startKeyIndex);
    ...
    if (isSafetyOrEmpty) {
      ...
      const [res1, res2] = await Promise.all([
        polishWithContentSplit(src1, raw1, genre, tone, glossary, additionalInstructions, apiKeys, model, depth + 1, startKeyIndex),
        polishWithContentSplit(src2, raw2, genre, tone, glossary, additionalInstructions, apiKeys, model, depth + 1, startKeyIndex)
      ]);
      return {
...
app.post("/api/translate-raw", async (req, res) => {
  try {
    const { text, genre, tone, glossary, apiKeys, model, startKeyIndex = 0 } = req.body;
    if (!text || typeof text !== "string") {
...
app.post("/api/polish-translation", async (req, res) => {
  try {
    const { sourceText, rawTranslation, genre, tone, glossary, additionalInstructions, apiKeys, model, startKeyIndex = 0, isExtractionEnabled } = req.body;
...
app.post("/api/align-chapter", async (req, res) => {
  try {
...
// 5. API: Kiểm tra trạng thái máy chủ khỏe mạnh
app.get("/api/health", (_req, res) => {
  res.json({ status: "ok", timestamp: new Date().toISOString() });
});

// 6. API: Trích xuất nhanh thuật ngữ (Tương thích với AutoTranslator)
app.post("/api/extract-glossary", async (req, res) => {
  try {
    ...
    const systemInstruction =
        ...
        "QUAN TRỌNG về trường 'vietnamese': Nếu tên nhân vật hoặc địa danh là phiên âm từ tiếng Anh hoặc ngôn ngữ phương Tây (ví dụ: 阿诗娜 = Athena, 盖伊 = Guy), hãy khôi phục TÊN GỐC TIẾNG ANH trong trường 'vietnamese'. " +
