...
export function buildEntitySchema(mode: 'checkLeftover' | 'analyze' | 'extract'): any {
  if (mode === 'checkLeftover') {
    return {
      type: "OBJECT",
      properties: {
        chinese: { type: "STRING", description: "Từ chữ Trung bị sót" },
        pinyin: { type: "STRING", description: "Phiên âm Hán-Việt chuẩn phù hợp" },
        vietnamese: {
          type: "STRING",
        ...
        note: { type: "STRING", description: "Mô tả vai trò/ý nghĩa dự kiến của đối tượng theo văn cảnh" }
      },
  ...
  if (mode === 'analyze') {
    return {
      type: "OBJECT",
      properties: {
        chinese: { type: "STRING", description: "Từ tiếng Trung gốc, ví dụ '萧炎' hoặc '乌坦城'" },
        pinyin: { type: "STRING", description: "Phiên âm Hán Việt chuẩn, ví dụ 'Tiêu Viêm' hoặc 'Ô Thản Thành'" },
        vietnamese: {
          type: "STRING",
        ...
        note: { type: "STRING", description: "Mô tả ngắn, ví dụ: 'Nhân vật nam chính, tư chất phi phàm' hoặc 'Nơi sinh ra của Tiêu Viêm'" }
      },
      required: ["chinese", "pinyin", "vietnamese", "type", "note"]
    };
  }
  ...
  return {
    type: "OBJECT",
    properties: {
      chinese: { type: "STRING", description: "Từ tiếng Trung gốc" },
      pinyin: { type: "STRING", description: "Phiên âm Hán-Việt chuẩn" },
      vietnamese: {
        type: "STRING",
      ...
      note: { type: "STRING", description: "Mô tả ngắn gọn vai trò/ý nghĩa" }
    },
    required: ["chinese", "pinyin", "vietnamese", "type", "note"]
  };
}
