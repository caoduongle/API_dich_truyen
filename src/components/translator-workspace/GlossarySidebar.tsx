        ...
        <div className="flex flex-wrap items-center gap-3">
          <label className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-400 cursor-pointer">
            <input
              ...
              className="rounded border-slate-750 border-slate-700 text-indigo-600 focus:ring-indigo-500 w-3.5 h-3.5 cursor-pointer bg-[#161f30]"
            />
            Chỉ từ trong chương
          </label>
          <input
        ...
        <div className="space-y-1.5">
          <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto pr-1">
            {visibleGlossary.map((item) => (
              <span
                key={item.id}
                className="inline-flex items-center gap-1.5 bg-[#161f30] border border-slate-800 rounded-lg px-2.5 py-1 text-xs shadow-sm text-slate-350 text-slate-300 font-sans hover:border-indigo-500/50 transition-all cursor-default"
              ...
              >
                ...
                <ChevronRight className="w-2.5 h-2.5 text-slate-650 text-slate-600 shrink-0" />
