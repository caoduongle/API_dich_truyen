...
export const ImportGuidelinesModal = React.memo(function ImportGuidelinesModal({
  ...
  return (
    <div id="md-uploader-zone" className="bg-slate-900/40 border border-slate-800 p-4 rounded-xl space-y-3.5 animate-slideUp backdrop-blur-md shadow-xl">
      ...
      <div
        ...
        className="border-2 border-dashed border-slate-800 hover:border-indigo-500/80 bg-slate-950/20 hover:bg-slate-900/30 p-6 rounded-lg text-center cursor-pointer transition-all flex flex-col items-center justify-center space-y-2 group"
      >
        ...
        <span className="text-[10px] text-slate-500 font-mono">Định dạng khuyên dùng: .md</span>
        <input type="file" accept=".md" ref={mdInputRef} onChange={handleMdImportFileChange} className="hidden" />
      </div>

      {isAnalyzingMd && (
        <div className="flex items-center gap-2 justify-center py-2 text-indigo-400 bg-indigo-950/40 border border-indigo-900/30 rounded-lg text-xs font-bold animate-pulse">
