...
export function DriveProgressWidget({
    ...
    return (
        <div
            ...
            className={`fixed bottom-4 right-4 z-50 bg-white border border-slate-200 rounded-xl shadow-2xl transition-all duration-300 overflow-hidden flex flex-col ${
                isMinimized ? 'w-72 sm:w-80 h-12' : 'w-80 sm:w-96 max-h-[420px] h-auto'
            }`}
        >
            <div className="bg-slate-900 text-white px-4 py-2.5 flex items-center justify-between shrink-0 select-none">
              ...
              <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                        ...
                        <span className="h-2 w-2 rounded-full bg-slate-400 font-bold"></span>
                    )}
            ...
            {!isMinimized ? (
                <div className="p-4 space-y-4 overflow-y-auto flex-1 text-slate-800">
                    <div className="space-y-1.5">
                        <div className="flex justify-between text-xs font-bold">
                            <span className="text-slate-500">Tiến trình chung:</span>
                            <span className="text-indigo-600 font-extrabold border border-indigo-100 px-1 rounded">
                {totalQueueLength > 0 ? Math.round((processedCount / totalQueueLength) * 100) : 0}%
                    ...
                    <div className="grid grid-cols-2 gap-3 bg-slate-50 p-2.5 rounded-lg border border-slate-150 text-[11px]">
                        <div className="space-y-0.5">
              <span className="text-slate-400 font-normal flex items-center gap-1">
                <Clock className="w-3.5 h-3.5 text-indigo-500" />
                Còn lại dự kiến:
              </span>
                            <strong className="text-slate-700 font-bold block truncate">
              ...
              <span className="text-slate-400 font-normal flex items-center gap-1">
                ...
                Request còn lại:
                    ...
                    {totalQueueLength > 0 && currentChapterTitle && (
                        <div className="bg-indigo-50/50 border border-indigo-100/75 p-2.5 rounded-lg text-xs space-y-1">
                            <span className="text-[10px] font-bold text-indigo-800 uppercase tracking-wider block">Chương hiện tại:</span>
                            <p className="font-extrabold text-indigo-950 truncate">{currentChapterTitle}</p>
                    ...
                    <div className="pt-2 border-t border-slate-100 flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="p-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg border border-slate-250 cursor-pointer"
                ...
                <div className="px-4 h-12 flex items-center justify-between text-xs font-medium text-slate-700">
              ...
              <span>Dịch sỉ: {processedCount}/{totalQueueLength} ({totalQueueLength > 0 ? Math.round((processedCount / totalQueueLength) * 100) : 0}%)</span>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button
                            ...
                            className="text-indigo-600 hover:text-indigo-800 font-bold hover:underline ml-1 cursor-pointer"
                        >
                            Mở rộng
                        </button>
                    </div>
