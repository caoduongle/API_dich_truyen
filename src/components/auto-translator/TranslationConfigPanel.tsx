  ...
  return (
    <div className="space-y-4 bg-parchment border border-parchment-2 p-5 rounded-md shadow-xs animate-fadeIn">
      ...
      <div className="space-y-2.5 pt-3 border-t border-parchment-2">
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            <span className="text-xs font-bold text-text-main flex items-center gap-1.5"><ListOrdered className="w-3.5 h-3.5 text-polish" /> Giới hạn phạm vi chương</span>
            <span className="text-[10px] text-text-muted font-normal mt-0.5">Dịch một phân khúc chương nhất định</span>
          </div>
          <button
            type="button"
            onClick={() => setRangeEnabled(!rangeEnabled)}
            className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none ${rangeEnabled ? 'bg-polish' : 'bg-parchment-2'}`}
          >
        ...
        {rangeEnabled && (
          <div className="space-y-3 animate-in slide-in-from-top-1 duration-200">
            ...
            <div className="bg-ink border border-parchment-2 rounded-[2px] px-3 py-2 text-[11px] text-text-main flex items-center justify-between">
              <span className="text-text-muted">Hàng đợi phân phối:</span>
              <strong className="text-polish font-bold">{safeEnd - safeStart + 1} chương</strong>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex justify-between items-center text-xs pt-1">
        <label className="font-bold text-text-main flex items-center gap-1.5"><Layers className="w-3.5 h-3.5 text-polish" /> Lượt chuốt văn học:</label>
        <span className="bg-ink border border-parchment-2 text-polish rounded-[2px] px-2.5 py-0.5 text-[10px] font-bold">{polishCycles} vòng</span>
      </div>
      ...
      <div className="flex items-center gap-2 pt-1">
        {[1, 2, 3, 4, 5].map(n => (
          <button
            ...
            className={`flex-1 py-1.5 rounded-[2px] text-xs font-bold border cursor-pointer transition-colors ${polishCycles === n ? 'bg-polish border-polish text-white shadow-xs' : 'border-parchment-2 bg-ink text-text-muted hover:bg-parchment-2'}`}
          >
            {n}
          </button>
        ))}
        ...
        <div className="flex items-center gap-2">
          {[1, 2, 3, 4, 5].map(n => (
            <button
              ...
              className={`flex-1 py-1.5 rounded-[2px] text-xs font-bold border cursor-pointer transition-colors ${concurrency === n ? 'bg-polish border-polish text-white shadow-xs' : 'border-parchment-2 bg-ink text-text-muted hover:bg-parchment-2'}`}
            >
              {n}
            </button>
          ))}
      ...
      <div className="space-y-1.5 pt-2">
        <label className="text-[11px] font-bold text-text-muted uppercase tracking-wider block">Yêu cầu bổ sung khi biên tập:</label>
        <input
          type="text"
      ...
      <div className="pt-3 pb-1 flex items-center justify-between border-t border-parchment-2 mt-2">
        <div className="flex flex-col pr-2">
          <span className="text-xs font-bold text-text-main">Tự động bỏ qua chương lỗi</span>
          <span className="text-[10px] text-text-muted font-normal">Bỏ qua chương lỗi và tiếp tục tiến trình</span>
        </div>
        <button
          type="button"
          onClick={() => setSkipFailedChapters(!skipFailedChapters)}
          className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none ${skipFailedChapters ? 'bg-polish' : 'bg-parchment-2'}`}
        >
      ...
      <div className="pt-3 pb-1 flex items-center justify-between border-t border-parchment-2 mt-1">
        <div className="flex flex-col pr-2">
          <span className="text-xs font-bold text-text-main">Rà soát từ mới khi dịch</span>
          <span className="text-[10px] text-text-muted font-normal">Tự động bóc tách từ vựng gối đầu</span>
        </div>
        <button
          type="button"
          onClick={() => setIsExtractionDuringTranslationEnabled(!isExtractionDuringTranslationEnabled)}
          className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none ${isExtractionDuringTranslationEnabled ? 'bg-polish' : 'bg-parchment-2'}`}
        >
